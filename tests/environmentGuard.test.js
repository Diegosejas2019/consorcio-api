...
describe('environmentGuard', () => {
  ...
  test('extrae el nombre de base desde URI local y Atlas', () => {
    ...
    expect(getMongoDatabaseName('mongodb+srv://user:pass@cluster.mongodb.net/gestionar_prod?retryWrites=true')).toBe('gestionar_prod');
  });

  test('bloquea production si no usa gestionar_prod', () => {
    process.env.NODE_ENV = 'production';

    expect(() => assertMongoEnvironment({
      uri: 'mongodb+srv://user:pass@cluster.mongodb.net/gestionar_qa',
    })).toThrow('NODE_ENV=production debe usar la base gestionar_prod');
  });

  test('bloquea qa si apunta a gestionar_prod', () => {
    process.env.NODE_ENV = 'qa';

    expect(() => assertMongoEnvironment({
      uri: 'mongodb+srv://user:pass@cluster.mongodb.net/gestionar_prod',
